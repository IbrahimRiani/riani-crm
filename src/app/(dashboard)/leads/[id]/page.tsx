...
import { ArrowLeft, Globe, Phone, Mail, MapPin, CalendarDays, Euro, User, UserCheck } from "lucide-react";
...
import type { Activity, Lead, Profile, TaskWithLead } from "@/types/crm";
...
export default async function LeadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: activities }, { data: tasks }, { data: allLeads }, { data: profiles }, role] = await Promise.all([
  ...
  const assigneeEmail = (profiles ?? []).find((p) => p.id === l.assigned_to)?.email ?? null;
    ...
    <div>
      ...
      <div className="mt-5 grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          <Card className="p-5">
            <LeadActions lead={l} isAdmin={role === "admin"} profiles={(profiles ?? []) as Profile[]} />
          </Card>
          <div>
        ...
        <div className="space-y-4">
          <Card className="p-5">
            ...
            <Row icon={UserCheck} label="Responsable" value={assigneeEmail ?? "Sin asignar"} />
