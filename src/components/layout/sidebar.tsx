...
export function Sidebar({ email }: { email?: string }) {
  ...
  const list = (
    <div className="flex h-full flex-col">
      ...
      <div className="border-t border-neutral-200 p-3 dark:border-neutral-800">
        <ThemeToggle />
        {email && (
          <p className="truncate px-3 pb-2 pt-2 text-xs text-neutral-500 dark:text-neutral-400" title={email}>
            {email}
          </p>
        )}
        <button
  ...
  return (
    <>
      {/* Desktop */}
      <aside className="hidden w-60 shrink-0 border-r border-neutral-200 bg-white dark:border-neutral-800 dark:bg-neutral-900 lg:block">
      ...
      <div className="sticky top-0 z-30 flex h-14 items-center justify-between border-b border-neutral-200 bg-white px-4 dark:border-neutral-800 dark:bg-neutral-900 lg:hidden">
        <span className="text-base font-bold text-neutral-900 dark:text-neutral-50">Ibra CRM</span>
        <button
      ...
      {open && (
        <div className="fixed inset-0 z-40 lg:hidden" role="dialog" aria-modal="true">
          ...
          <div className="absolute left-0 top-0 h-full w-64 bg-white shadow-xl dark:bg-neutral-900">{list}</div>
        </div>
      )}
    </>
  );
